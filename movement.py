...
def a_star(start: Coordinate, goal: Coordinate, rows: int, cols: int,
           obstacles: Set[Coordinate]) -> List[Coordinate]:
    """Classic 4‑neighbour A* on a rectangular grid."""
    ...
    raise RuntimeError("No path found (check obstacles)")


def elastic_band(path: List[Coordinate], obstacles: Set[Coordinate],
                 iterations: int = 200, spring: float = 0.3, repel: float = 2.0,
                 obstacle_radius: float = 1.5) -> List[Tuple[float, float]]:
    """Light‑weight TEB‑style spatial optimiser over an A* seed path."""
    ...
    for _ in range(iterations):
        for i in range(1, len(pts) - 1):
            ...
            # Spring force pulls toward midpoint between neighbours
...
def plan_path(rows: int, cols: int, start: Coordinate, goal: Coordinate,
              obstacles: Set[Coordinate], smooth: bool = True) -> List[Tuple[float, float]]:
    """Return a path; A* grid path then optional TEB smoothing."""
    grid_path = a_star(start, goal, rows, cols, obstacles)
    if smooth:
        return elastic_band(grid_path, obstacles)
